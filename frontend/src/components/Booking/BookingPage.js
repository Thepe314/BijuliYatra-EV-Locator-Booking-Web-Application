...
import { 
  ...
  BatteryCharging, AlertCircle, Loader, CheckCircle2 
} from 'lucide-react';
import { toast, ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';   // ← This line is required
...
export default function BookingPage() {
  ...
  useEffect(() => {
    const loadStation = async () => {
      ...
      try {
        ...
        toast.success(`Welcome to ${foundStation.name}`, { icon: <Zap className="w-5 h-5" /> });
      } catch (err) {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8080/stations/${stationId}/bookings?date=${formData.date}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/ev-owner/dashboard'), 1500); 
    } catch (err) {
      const msg = err.response?.data || "Slot no longer available. Please choose another time.";
  ...
  const rate = formData.connectorType === 'DC Fast' 
  ...
  return (
    <>
      {/* Toast Container – only one needed in the whole app */}
      <ToastContainer
        position="top-center"
        autoClose={3000}
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl p-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {station.dcFastChargers > 0 && (
                      <label className="cursor-pointer">
                        <input type="radio" name="connectorType" value="DC Fast" checked={formData.connectorType === 'DC Fast'}
                          onChange={(e) => setFormData({...formData, connectorType: e.target.value})} className="sr-only" />
                        <div className={`p-6 border-2 rounded-xl transition-all ${formData.connectorType === 'DC Fast' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200'}`}>
                          ...
                          <p className="text-sm text-gray-600">Up to 150 kW • {station.dcFastChargers} ports</p>
                    ...
                    {station.level2Chargers > 0 && (
                      <label className="cursor-pointer">
                        <input type="radio" name="connectorType" value="Level 2" checked={formData.connectorType === 'Level 2'}
                          onChange={(e) => setFormData({...formData, connectorType: e.target.value})} className="sr-only" />
                        <div className={`p-6 border-2 rounded-xl transition-all ${formData.connectorType === 'Level 2' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200'}`}>
                          ...
                          <p className="text-sm text-gray-600">22 kW • {station.level2Chargers} ports</p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {timeSlots.map(slot => {
                      const slotDateTime = formData.date ? `${formData.date}T${slot}:00` : null;
                      ...
                      if (formData.date && slotDateTime) {
                        ...
                        const end = new Date(start.getTime() + parseInt(formData.duration) * 3600000);
                        let check = new Date(start);
                        while (check < end) {
                          const checkKey = check.toISOString().slice(0, 16);
                      ...
                      const disabled = !formData.date || isBlocked || durationBlocked;
                      ...
                      return (
                        ...
                        >
                          {slot}
                          {disabled && <div className="absolute inset-0 rounded-xl bg-black opacity-10"></div>}
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex gap-6 mt-6 text-sm text-gray-600">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded bg-gray-200 line-through"></div>
                ...
                >
                  {submitting ? (
                    ...
                    <>
                      ...
                      Confirm Booking • Rs. {totalCost.toLocaleString()}
                    </>
                  )}
                </button>
              </form>
                ...
                <div className="space-y-5 text-lg">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold">{formData.duration} hour{formData.duration > 1 ? 's' : ''}</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-emerald-50 to-cyan-50 rounded-xl p-6 mt-6">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-3xl font-bold text-emerald-600">Rs. {totalCost.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </div>
